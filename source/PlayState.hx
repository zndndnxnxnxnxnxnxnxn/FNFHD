...
/*		// Add Kade Engine watermark
	...
	override public function update(elapsed:Float)
	{
		...
		switch(Config.accuracy){
			case "none":
				scoreTxt.text = "HD Ported by zack and klav" + "Score:" + songScore;
			default:
				scoreTxt.text = "Score:" + songScore + " | Misses:" + misses + " | Accuracy:" + truncateFloat(accuracy, 2) + "%" + "HD Ported by Z & K";
		}
		...
		if (generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if(Config.newInput && daNote.isSustainNote && daNote.mustPress){
					...
					if(daNote.prevNote.wasGoodHit){ //DEAR GOD WHY DOES THIS HAVE A DIFFERENT CONTROL VAR FROM THE OTHERS DJEIEBIENI
						
						#if mobile
						var upP = mcontrols.UP;
						var rightP = mcontrols.RIGHT;
						var downP = mcontrols.DOWN;
						var leftP = mcontrols.LEFT;
						#else
						var upP = mcontrols.UP;
						var rightP = mcontrols.RIGHT;
						var downP = mcontrols.DOWN;
	...
	function noteMiss(direction:Int = 1, ?healthLoss:Float = 0.04, ?playAudio:Bool = true, ?skipInvCheck:Bool = false):Void
		...
		function goodNoteHit(note:Note):Void
		{
			...
			//Bruh I swear this broke the entire game lmao

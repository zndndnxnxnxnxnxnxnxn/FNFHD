...
class AutoOffsetState extends MusicBeatState
{
	...
	override function create() {	
		...
		if(!easterEgg){
			//Init BG
			bg = new FlxSprite(0, 0).loadGraphic(Paths.image('offsetBG', "fpsPlus"));
			...
			//Init Speakers
			speakers.frames = Paths.getSparrowAtlas('speaker', "fpsPlus");
			...
			//Init BG
			bg = new FlxSprite(0, 0).loadGraphic(Paths.image('offsetBG-pixel', "fpsPlus"));
			...
			//Init Speakers
			speakers = new FlxSprite(0, 0).loadGraphic(Paths.image('speaker-pixel', "fpsPlus"), true, 102, 52);
		...
		FlxG.camera.fade(FlxColor.BLACK, 0.5, true, function(){
			...
			FlxG.sound.playMusic(Paths.music("offsetSong" + (easterEgg?"-pixel":""), "fpsPlus"), 1, false);
	...
	function exit():Void {
		...
		ending = true;
		Config.write(offsetCalc, Config.accuracy, Config.healthMultiplier, Config.healthDrainMultiplier, Config.betterIcons, Config.downscroll, Config.newInput, Config.noteGlow, Config.noRandomTap, Config.disableCutscenes);
